...
| G4 | **Completed-but-unpaid report**: the EOD audit is possible but manual (scan the queue) | Medium | Add a filter/report view: completed JOs without `service_invoice_no`, aging by days. Cashier's collections worklist. |
| G5 | **Admin queue scale**: loads everything, no status filter/pagination — fine now, strained at 50–60 JO/day within weeks | Medium | Segmented status filter + "this week" default + pagination. |
...
| G7 | **Staff password reset**: `@ktc-staff.local` accounts can't use email reset; owner has no reset RPC — a locked-out checker needs a new account | Medium | `reset_staff_password(username, new_pw)` owner-only RPC. |
