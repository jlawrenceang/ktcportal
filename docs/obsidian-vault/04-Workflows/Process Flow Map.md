...
| G11 | **Go-live legal/testing**: Customer Agreement still template (counsel), ST02 not yet executed, Playwright Phase 2 pending | Gate for launch | **ST02 script ready** (`docs/smoke-test-02-portal.md`, 8 lanes covering sessions 4–10) — execute on live. Phase 2 needs `E2E_SERVICE_ROLE_KEY` + ideally a test project (`e2e/README.md`; use non-MFA test accounts). Counsel → bump `AGREEMENT_VERSION`. |
