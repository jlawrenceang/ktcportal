...
| Metric | Count |
|--------|-------|
| App version | `src/version.ts` = **`v1.6.13`** (live on `portal.ktcterminal.com`). Footers show version + git commit + build date |
| Migrations | **150 files**, `0001_init` … **`0159_release_desk_reason_required`** — all applied + tracked in `public._migrations`. Numbering is split across a **portal lane** and a **fuel lane** (fuel = `0135`/`0140`/`0150`), so there are gaps; the portal lane now runs contiguously through `0159` |
