...
| Metric | Current |
|---|---|
| App version | `src/version.ts` = `v2.0.11` |
| Migrations | `0001_init` through `0232_native_push_tokens`, applied/tracked on KTC prod. Recent active spine: ADR-0037 charge/payment-order cutover (`0203`-`0222`), hardening `0228`-`0231`, native push scaffold `0232`. |
...
| Current manual smoke | `docs/smoke-test-08-go-live.md` for v2.0.11 / migration 0232. ST05/ST06/ST07 are closed legacy stubs. |
