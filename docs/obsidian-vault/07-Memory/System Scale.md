...
| Metric | Count |
|--------|-------|
| App version | `src/version.ts` = **`v1.6.77`** (live on `portal.ktcterminal.com`). Footers show version + git commit + build date |
| Migrations | **178 files**, `0001_init` … **`0187_breaktest_hardening_medium`** — all applied + tracked in `public._migrations`. Numbering is split across a **portal lane** and a **fuel lane** (fuel = `0135`/`0140`/`0150`), so there are gaps; the portal lane now runs contiguously through `0187`. Recent big runs: consignee approval gate + CIS (`0165`–`0169`), the **ADR-0035 ops overhaul** (`0170`–`0177`: SoD · auto-complete · queue lifecycle · priority + re-X-ray lanes · ops→cashier billing · invoice-gated payment), the **whole-app audit closure** (`0178`–`0183`), then `0184` (owner-failsafe email-keyed backstop), `0185` (recoverable rejected consignee request), `0186` (break-test critical+high fixes — supplement/rexray/completion/payment-guard) + `0187` (break-test security/integrity hardening — RPS guards, terminal-reject retirement, checker accept-gate, file caps, RLS gates). Earlier (`0160`–`0164`): support_contact SELECT, Google-OAuth finish-registration, consent enforcement, pending verify-only lockdown, disposable-email block |
