...
- [ ] **Online payment (non-gated)** — optional "Pay online" to settle a JO (skip the counter); JO stays processable without payment. Shows an **in-app computation page** (the breakdown above = Total to pay), **not** the official BIR invoice. Decide gateway (PayMongo / Xendit / Maya / DragonPay — GCash + Maya + cards), receipt/OR handling, `payment_status` / `paid_at` / reference on `job_orders`, admin reconciliation.
