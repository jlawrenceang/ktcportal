...
1b. **Online payment (non-gated, convenience).** Optional **"Pay online"** to settle a JO and skip the counter/line — JO **still processable without payment**. **Clarified 2026-06-11:** online payment shows an **in-app computation page** (X-ray rate × containers + 12% VAT + admin/service fee + print fee = **Total to pay**) — a "what you owe" summary, **not** the official BIR invoice (filled at the cashier). Open: gateway (PH — PayMongo / Xendit / Maya / DragonPay; GCash + Maya + cards), receipt/OR handling, `payment_status` / `paid_at` / reference on `job_orders`, admin reconciliation. Build after rate/fee config exists.
