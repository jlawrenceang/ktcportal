...
> **⚠️ SUPERSEDED IN PART by the v2.0.0 X-ray billing cutover (ADR-0037, migrations `0203`–`0222`).** The **billing legs** of these lanes — Lane **A** (invoice → pay → auto-complete), Lane **G** (request→bill charges via `request_supplement`/`bill_supplement`), Lane **H** (walk-in + RPS-at-window), and the `record_service_invoice` / `review_payment` / supplement RPCs in Lane **J** — are **DELETED**. Billing is now one uniform **`charges`** table: the customer pays **per charge, inline** in My Job Orders (`JobOrderCharges` → *Pay this charge*); the cashier collects on the **Payment Order desk** (`/admin/payment-orders`, PWA `/app/payment-orders` — the **CashierStation** `/admin/cashier` + `/app/cashier` and the customer Pay page `/job-order/:id/pay` are gone); add-ons are `add_charge` → `approve_charge` (maker-checker), each confirmed only against a **FINAL ERP+BIR invoice** (`record_charge_invoice` → `confirm_charge_payment` / `confirm_payment_order`); completion is **one rule** (all services done AND every billed charge confirmed); the **serving number** resets **monthly** as **`YYMM-XXXX`**; customer self-cancel is blocked once a charge leaves pristine and a new **admin-only** `admin_cancel_job_order` exists. **For the current billing / cashier / serving script, run [ST06](smoke-test-06-portal.md).** The **non-billing** lanes here (full-spine X-ray, hold/reject/cancel, priority, re-X-ray, consignee request, the security walls J, the concurrency pass K) still apply.
