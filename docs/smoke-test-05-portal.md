...
**⚠ One finding (see Defect D-01):** the **release-desk hold/reject reason note is NOT server-enforced** — `verify_release_order`, `confirm_release_payment`, and `confirm_release_supplement_payment` store `nullif(trim(p_note),'')` (a blank reason → NULL) with no RAISE, whereas the **JO side** (`review_payment`/`review_supplement_payment`) *does* require a non-empty note. ST05's I-2/I-4 guardrail cells were corrected to match the server, and the asymmetry is logged as a low-severity gap.
