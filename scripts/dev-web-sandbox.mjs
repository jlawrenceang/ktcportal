...
if (ref === 'mdlnfhyylvapzdubhyic') {
  console.error('Refusing to run sandbox dev server against the production Supabase ref.')
  process.exit(1)
}
...
console.log(`Starting sandbox dev server against Supabase ref: ${ref}`)
run('npx', ['vite', '--mode', 'sandbox', '--host', '127.0.0.1', '--port', '3000', '--strictPort'], env)
