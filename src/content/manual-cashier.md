# KTC Online Portal — Cashier Guide

Your station is the **Job Orders** queue — you land there at sign-in. Two jobs: review online payment proofs, and record Service Invoice numbers. (Exactly which buttons you see is set by the owner's role gates; this guide covers the standard cashier setup.)
...
- If your account has 2FA enrolled you'll also enter a 6-digit code from the authenticator app.

## 2 · Reviewing payment proofs
...
- the **control number** — OR-INV-… (cash) or BI-INV-… (billed/credit), and
- the **printed pad serial** — the number on the physical pad, e.g. 001323 (keep the leading zeros).

Both are validated (garbage is rejected with a hint) and saved together. The order then shows **PAID** (OR) or **BILLED** (BI), and the customer sees the printed number on their order.
...
- Never share your password; the admin can reset it anytime from Settings.

---

*Anything that looks wrong with an order itself (statuses, services, holds) is handled by an admin — flag it rather than working around it.*
