...
const n = async (q: PromiseLike<{ count: number | null; error: { message: string } | null }>) => {
  ...
  if (error) throw error // surface a failed count instead of a silent 0 on the tile
  return count ?? 0
}
...
export default function Dashboard() {
  ...
  const load = useCallback(() => {
    ...
    // Both blocks resolved together → one error gate, no cross-clearing race
    // between the stats and queue setLoadError calls. A stats count error rejects
    // statsP → the outer .catch fires; a queue read error is caught via `err`.
    Promise.all([statsP, queueP]).then(([s, [a, c, p, tk, x]]) => {
      const err = a.error || c.error || p.error || tk.error || x.error
      if (err) { setLoadError(err.message); return }
      setLoadError(null)
      const [pendingAccounts, pendingConsignees, brokers, consignees, jobOrders, paymentProofs, supportTickets, xrayVans] = s
