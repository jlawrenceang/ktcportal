...
import { chargeState, chargeHasProofToReview } from '../lib/charges'

interface AdminJobOrder extends JobOrder {
  broker?: { full_name: string | null; email: string | null; contact_number: string | null } | null
  // Post-cutover billing state (ADR-0037): the unified `charges` source replaces
  // the retired base/RPS/supplement/service-invoice columns. Read via chargeState.
  charges?: { bill_status: string; payment_status: string }[] | null
}
...
const SELECT =
  'id, jo_number, entry_number, consignee_id, vessel_name, voyage_number, vessel_visit, status, priority_status, is_rexray, rexray_status, rexray_billable, parent_job_order_id, admin_note, customer_note, rejected_recoverable, xray_performed_at, rps_status, completed_at, archived_at, created_at, last_customer_edit_at, broker:customers(full_name, email, contact_number), consignee:consignees(code, name), lines:job_order_lines(container_number, service_request), serving:serving_numbers(service_line, serving_no, week_start, vacated_at), completions:service_completions(service_line, completed_at), charges:charges(bill_status, payment_status)'
...
// Queue views (G4/G5): server-side filters + pagination.
type Filter = 'open' | 'completed' | 'closed' | 'archived' | 'all'
...
export default function AllJobOrders({ app = false }: { app?: boolean }) {
  ...
  // and no nested prompt (note / message) is showing — those own Escape.
  useEffect(() => {
    if (!selectedId || modal || msgOrder) return
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape') setSelectedId(null) }
    document.addEventListener('keydown', onKey)
    return () => document.removeEventListener('keydown', onKey)
  }, [selectedId, modal, msgOrder])
  ...
  // Admin cancel (ADR-0037): voids a still-active order with a required reason
  // (logged server-side). Distinct from reject — used for duplicates / withdrawals.
  async function cancelOrder(o: AdminJobOrder) {
    const reason = window.prompt(t('Cancel this order? Enter a reason (the customer and the timeline will see it).'), '')
    if (reason === null) return
    if (!reason.trim()) { alert(t('A reason is required to cancel an order.')); return }
    setBusyId(o.id)
    const { error } = await supabase.rpc('admin_cancel_job_order', { p_id: o.id, p_reason: reason.trim() })
    setBusyId(null)
  ...
  function PaymentPill({ o }: { o: AdminJobOrder }) {
    const state = chargeState(o.charges)
  ...
  function SecondaryChips({ o }: { o: AdminJobOrder }) {
    ...
    return (
      <>
        ...
        {chargeHasProofToReview(o.charges) && (
          <span className="ktc-chip ktc-chip--warning" style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}
  ...
  function OrderActions({ o }: { o: AdminJobOrder }) {
    ...
    // Complete is two-gated: every service done AND no outstanding balance. The
    // billed charges must all be confirmed (chargeState 'paid'); staff_transition_order
    // re-checks the gate server-side.
    if (can('complete_orders') && o.status === 'processing' && chargeState(o.charges) === 'paid' && serviceProgress(o).every((p) => p.done)) {
    ...
    // Admin cancel — voids a still-active order with a required reason (ADR-0037).
    if (can('hold_reject_orders') && ['submitted', 'processing', 'on_hold'].includes(o.status)) {
      menu.push(<button key="cancel" style={btn('danger')} disabled={isBusy} onClick={() => void cancelOrder(o)}>{t('Cancel order')}</button>)
    }
  ...
  // Shared body (notes, lines, edit form, release tracks, actions, timeline).
  function OrderBody({ o }: { o: AdminJobOrder }) {
    return (
      <>
        ...
        {/* Two-gate release status: payment (charges) + X-ray → cleared for release */}
  ...
  function OrderCard({ o }: { o: AdminJobOrder }) {
    ...
    return (
      <button type="button" onClick={() => setSelectedId(o.id)}
        ...
        {/* (a) Header row: JO# · status · balance pill */}
          ...
          <div style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            ...
            {can('process_job_orders') && (filter === 'completed' || filter === 'all') && (
              <button type="button" className="ktc-btn-secondary ktc-btn--sm" style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }} disabled={archiving}
                title={t('Archives every completed & fully-paid order. Also runs automatically every Monday.')}
          ...
          <div className="ktc-modal-backdrop" onClick={close}>
            <div className="ktc-glass ktc-modal-panel" onClick={(e) => e.stopPropagation()}
              ...
              <div style={{ overflowY: 'auto', padding: '16px 20px' }}>
                ...
                {/* Full secondary / aging / progress chips */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap', marginTop: 8 }}>
                  <SecondaryChips o={o} />
                </div>
