...
export default function Security() {
  ...
  async function startEnroll() {
    ...
    setBusy(false)
    if (err || !data) { setError(err?.message ?? 'Could not start enrollment.'); return }
  ...
  async function confirmEnroll(e: FormEvent) {
    ...
    if (cErr || !ch) { setBusy(false); setError(cErr?.message ?? 'Could not start the check.'); return }
    const { error: vErr } = await supabase.auth.mfa.verify({
      factorId: enrolling.factorId, challengeId: ch.id, code: code.trim(),
    })
    setBusy(false)
    if (vErr) { setError("That code didn't match — scan the QR again or re-type the code."); setCode(''); return }
    setEnrolling(null)
    setNotice('✓ Two-factor authentication is ON. From now on, signing in asks for a code from your app.')
    await loadFactors()
  }
  ...
  return (
    <AdminShell>
      <div className="ktc-glass" style={{ padding: 28, maxWidth: 620 }}>
        <h1 className="ktc-title">Two-factor authentication</h1>
        <p className="ktc-label" style={{ marginTop: 8, lineHeight: 1.65 }}>
          ...
          to your sign-in. Once enabled it's enforced <b>server-side</b> — a stolen password alone
          can't reach any staff function.
        </p>
        ...
        {loading ? (
          ...
          <div style={{ marginTop: 20, display: 'grid', gap: 14 }}>
            <b style={{ fontSize: 14.5 }}>1 · Scan this QR code with your authenticator app</b>
            <img src={enrolling.qr} alt="TOTP enrollment QR code" style={{ width: 190, height: 190, borderRadius: 12, border: '1px solid var(--glass-brd)', background: '#fff', padding: 8 }} />
            <span className="ktc-label" style={{ fontSize: 12.5 }}>
              Can't scan? Enter this key manually: <code className="ktc-mono" style={{ userSelect: 'all' }}>{enrolling.secret}</code>
            </span>
            <form onSubmit={confirmEnroll} style={{ display: 'grid', gap: 10, maxWidth: 280 }}>
              <b style={{ fontSize: 14.5 }}>2 · Enter the 6-digit code it shows</b>
              <input
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button type="button" className="ktc-btn-secondary" onClick={() => void cancelEnroll()} style={{ padding: '11px 18px' }}>Cancel</button>
              </div>
            </form>
          </div>
        ) : verified.length > 0 ? (
          <div style={{ marginTop: 20, display: 'grid', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 16px', borderRadius: 12, background: 'hsl(145 60% 96%)', border: '1px solid hsl(145 50% 80%)' }}>
              <span className="ktc-chip ktc-chip--success">ON</span>
              <span style={{ fontSize: 14 }}>Two-factor authentication is active on this account.</span>
            </div>
            {confirmRemove ? (
              <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap', fontSize: 13.5 }}>
                <span style={{ fontWeight: 600, color: 'var(--acc-2)' }}>Remove 2FA and go back to password-only?</span>
                ...
                <button type="button" className="ktc-link" onClick={() => setConfirmRemove(false)}>Keep it on</button>
              </div>
            ) : (
